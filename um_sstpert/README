...
This "um_sstpert" module provides a Python extension from the UM sstpert
library, and an associated utility to produce SST perturbation files

(note that the UM sstpert library itself is obtainable via a UM licence 
and must be installed separately).

Building the extension
======================
The first step is to build the extension module; you must link against the 
UM sstpert library here, so it should be installed beforehand (the UM 
...
path to a variable (as we will need it multiple times):
    ...
    DIR=/path/to/the/um/sstpert/library

(The above path should be the one containing the "include" and "lib" 
subdirectories, with the "libum_sstpert.so" shared object present in "lib")
...
this package:
    ...
    echo $PWD/lib > ~/.local/lib/python2.7/site-packages/um_sstpert.pth
...
site also has a central install of the "um_sstpert" module this will override
